# Sources

Campaign Custom Rules: class revisions
- http://legendspring.wikidot.com/category:classes

Campaign Custom Rules: feat revisions
- http://legendspring.wikidot.com/feats

Core Brawler rules (d20pfsrd)
- https://www.d20pfsrd.com/classes/hybrid-classes/brawler

Campaign Custom Lizardfolk rules:
- http://legendspring.wikidot.com/races#toc20

Core Lizardfolk rules:
- https://www.d20pfsrd.com/races/other-races/more-races/standard-races-1-10-rp/lizardfolk-8-rp

Campaign Custom Combat Maneuvers:
- http://legendspring.wikidot.com/special-attacks

---

# Dockside Brawler Progression Plan (Levels 1-10)

## Build Overview
Lizardfolk Brawler with campaign modifications:I ma
- Brawler's Flurry at level 2
- Brawler's Slam at level 3 (campaign feature)
- Legendspring OCM/DCM combat system
- Dockside Brawler trait: +1 damage with brass knuckles and improvised weapons

**Race:** Lizardfolk (Legendspring campaign version)
- +2 Str, +2 Con, -2 Int
- Medium, 30 ft., swim 15 ft.
- Inferior, Low-light Vision
- Natural Armor +1, Bite 1d3 (primary; secondary at -5 if used with manufactured weapon attacks)
- Racial Talents: Hold Breath, Chameleon (+4 Stealth, +8 when still), Rock Lizard (climb 15 ft.)
- Racial Skills: +2 Acrobatics, +2 Swim
- Languages: Lizardfolk (native), trade common or maritime common (free), draconic/koboldic/nearby human (Int bonus)
- **Favoured Classes (by RAW): Barbarian, ranger, or fighter** — see note below

**Favoured Class:** Barbarian. Lizardfolk don't list Brawler as a favoured class, and the GM declined to approve it. Pick Barbarian as favoured class at level 1. The standard **+1 HP** favoured class benefit applies when taking Barbarian levels. No favoured class benefit applies during Brawler levels 1-5.

### Ability Scores (Rolled)

**Raw rolls:** 19, 13, 11, 12, 11, 9

| Stat | Raw | Racial | Final | Mod | Notes |
|------|-----|--------|-------|-----|-------|
| **STR** | 19 | +2 | **21** | +5 | Primary. Exceptional at level 1 |
| **DEX** | 13 | — | **13** | +1 | Meets Improved Grapple & Defensive Mobility prereqs. Adds to melee attacks per campaign rules |
| **CON** | 12 | +2 | **14** | +2 | HP, Fort saves, 6 Rage rounds/day (4 base + 2 CON) |
| **WIS** | 11 | — | **11** | +0 | Will saves. No bonus but no penalty |
| **INT** | 11 | -2 | **9** | -1 | Brawler's Cunning handles combat feat prereqs. Affects skill proficiencies |
| **CHA** | 9 | — | **9** | -1 | Dump. No class needs |

### Stat Priorities CAMPAIGN-SPECIFIC

Melee attack uses **BAB + STR + DEX** (not just STR). With STR 21 and DEX 13, the attack bonus at level 1 is **+7** (BAB +1, STR +5, DEX +1).

| Priority | Stat | Why | Your Score |
|----------|------|-----|------------|
| **Primary** | STR | Damage, OCM, attack rolls, CMC | 21 (+5) — outstanding |
| **Primary** | DEX | Attack rolls, AC, DCM, CMC, Reflex, initiative | 13 (+1) — meets all feat prereqs |
| **Secondary** | CON | HP, Fort saves, Rage rounds | 14 (+2) — solid |
| **Tertiary** | WIS | Will saves | 11 (+0) — needs Iron Will |
| **Dump** | INT | Combat Reflexes not viable without investment | 9 (-1) — Brawler's Cunning covers feats |
| **Dump** | CHA | No class needs | 9 (-1) |

### Ability Score Increases EVERY 2 LEVELS

ASI at levels 4, 6, 8, 10 — four increases by level 10 instead of core's two.

**Recommended ASI path (with actual stats):**
1. Level 4: +1 DEX → **DEX 14 (+2)** — bumps attack, AC, DCM, Reflex, initiative
2. Level 6: +1 STR → **STR 22 (+6)**
3. Level 8: +1 STR → **STR 23 (+6)**
4. Level 10: +1 CON → **CON 15 (+2)** — more HP, more Rage rounds

By level 10: STR 23, DEX 14, CON 15 — attack bonus of +14/+9 (+9 BAB, +6 STR, +2 DEX, -3 flurry) with brass knuckles before Power Attack.

### Armour

Studded Leather: +3 AC, **DR 1/—**, Max DEX +5, ACP -1. Works fully with Brawler AC Bonus (+1 dodge at level 4, +2 at level 9). The universal DR stacks with any other DR acquired.

### Progression System: Standard Multiclassing

The campaign offers both standard multiclassing and polyclassing. Polyclass secondary levels are cheaper in XP but grant **no BAB, saves, hit dice, or skills** — only class features. Since both Brawler and Barbarian are full-BAB martials, standard multiclassing preserves attack bonus, save, and HP progression. The ~9,000 XP premium for the first dip level is worth keeping full martial scaling.

Additional Barbarian levels beyond the level 6 dip can be taken whenever Rage powers are desired — standard multiclass costs apply, but Brawler remains the primary class throughout.

### XP Progression (Standard Multiclass)

| Char Lvl | Class Level | XP This Level | Total XP |
|----------|------------|---------------|----------|
| 1 | Brawler 1 | — | 0 |
| 2 | Brawler 2 | 3,000 | 3,000 |
| 3 | Brawler 3 | 4,500 | 7,500 |
| 4 | Brawler 4 | 6,500 | 14,000 |
| 5 | Brawler 5 | 9,000 | 23,000 |
| 6 | Barbarian 1 | 12,000 | 35,000 |
| 7 | Brawler 6 | 18,000 | 53,000 |
| 8 | Brawler 7 | 24,000 | 77,000 |
| 9 | Brawler 8 | 38,000 | 115,000 |
| 10 | Brawler 9 | 45,000 | 160,000 |

## Combat Identity
- Flurry-based pressure fighter
- Slam-based burst damage option
- Grapple/control specialist in melee chaos
- Improvised weapon scrapper

## Custom Background/Trait

**Dockside Brawler:** You grew up on the dangerous docks of one of the Knuckles' numerous ports or anchorages, and quickly learned that fists often make a better point than fancy words, especially when used the right way. You've always preferred action to talk, anyway. You gain a +1 trait bonus on damage rolls with brass knuckles (Advanced Player's Guide 176) and improvised weapons. While drinking at a tavern called the Fox and Falcon in Noamin last night, you got into a brawl with some of the other patrons. You handled yourself well, but a group of rough-looking characters ganged up on you and overwhelmed you, knocking you unconscious. Fortunately, you were able to conceal your brass knuckles when you were press-ganged, and you begin the campaign with them, regardless of your starting circumstances.

---

# LEVEL 1 — BRAWLER 1

## Class Features
- Improved Unarmed Strike (1d6 Medium)
- Martial Flexibility (move action, 1 feat, 4 rounds/day)
- Brawler's Cunning (Int 13 for combat feat prereqs)

## Feat (Level 1)
**Recommended: Power Attack**
- Prereqs: Str 13, BAB +1 [yes] (STR 21, BAB +1)
- Attack at level 1: **+7** (BAB +1, STR +5, DEX +1), damage: 1d6+5 unarmed / 1d3+6 with brass knuckles (+1 trait)
- With Power Attack: **+6** for **1d6+7** (unarmed) or **1d3+8** (brass knuckles)

**Alternatives:**
- Combat Expertise (if Improved Expertise Maneuvers is planned for level 2; Int 13 is satisfied by Brawler's Cunning)

---

# LEVEL 2 — BRAWLER 2

## Class Features
- Brawler's Flurry (as Two-Weapon Fighting when full-attacking with unarmed/close weapons)
- **Bonus Combat Feat**

## Feat (Bonus Combat Feat)
Priority options:
1. **Improved Grapple** — Prereqs: Dex 13, Improved Unarmed Strike [yes]. No AoO on grapple maneuvers, +2 to perform & +2 CMC vs grapple
2. **Improved Overpowering Maneuvers** — Prereqs: Str 13, Power Attack, BAB +1 [yes]. No AoO on bull rush/overrun/sunder, +2 to perform & +2 CMC vs those maneuvers
3. **Improved Expertise Maneuvers** — Prereqs: Int 13 (satisfied by Brawler's Cunning), Combat Expertise. Only viable if Combat Expertise was taken at level 1 instead of Power Attack

---

# LEVEL 3 — BRAWLER 3

## Class Features
- **Maneuver Training 1** (flurry attack penalty reduced to -1)
- **Brawler's Slam** (1d8, 1.5× STR scaling) — full-round action

## Feat (Level 3)
**Recommended: Defensive Combat Training**
- No prereqs. +2 competence bonus to DCM.
- **Why not Catch Off-Guard?** The campaign version of Catch Off-Guard requires BAB +4. At level 3, BAB is only +3. You'll take Catch Off-Guard at level 5 instead.

---

# LEVEL 4 — BRAWLER 4

## Class Features
- **AC Bonus +1** (dodge bonus; campaign option increases this by 1.5× if chosen as favoured class benefit)
- **Knockout 1/day** (fort save or unconscious; melee attack, must declare before rolling)
- ~~Bonus Combat Feat~~ — **Brawler 4 does NOT grant a bonus feat.** Core progression is Brawler levels 2, 5, 8, 11, 14, 17, 20.

---

# LEVEL 5 — BRAWLER 5

## Class Features
- **Bonus Combat Feat**
- **Brawler's Strike (magic)** — unarmed strikes count as magic for DR
- **Close Weapon Mastery** — damage with close weapon group = unarmed strike damage

## Feat (Level 5 — regular)
**Recommended: Catch Off-Guard**
- Prereqs: BAB +4 [yes] (BAB is +5 at this level)
- Benefit: Only -1 penalty with improvised melee weapons (normally -4). Unarmed opponents flat-footed to first improvised attack.

## Feat (Bonus Combat Feat)
Take whichever priority option from level 2 was not taken, or:
- **Defensive Mobility** — Prereqs: Dex 13, Defensive Combat Training [yes] (taken at level 3). Roll DCM to avoid AoO when moving out of threatened squares.

---

# LEVEL 6 — BARBARIAN 1
**XP:** +12,000 (35,000 total) — standard multiclass

## Class Features
- **Rage** (burst combat mode: +4 Str/Con, +2 Will, -2 AC)
- Fast Movement (+10 ft. in medium or lighter armor)
- Note: campaign Barbarian gets DR 1/— at level 4, a second Rage Power at 4, and Uncanny Dodge at 2 — reachable if taking additional Barbarian levels (see below)

---

# LEVEL 7 — BRAWLER 6

## Class Features
- **Martial Flexibility upgrade:** swift action to gain 2 combat feats simultaneously (instead of move action for 1)

## Feat (Level 7 — regular)
**Recommended: Iron Will**
- +2 to Will saves. No prereqs.
- **Why this over Combat Reflexes:** The campaign version of Combat Reflexes gives AoOs = lower of Dex or Int. As a Lizardfolk with -2 Int, investing 2+ ASIs in INT would be needed just to get 1 extra AoO. Meanwhile, Brawler Will saves are poor (+2 base at level 6) and Inferior Vision gives -4 vs illusions. Iron Will patches a key weakness. If the Battered rule is enforced, Rage's +2 morale Will plus Iron Will's +2 gives a net +2 Will even when Battered and raging. If Battered is not enforced (as in previous campaign), Iron Will is still strong but Toughness becomes equally attractive.
- **Note from previous campaign:** GM did not enforce Battered penalties — if this continues, both Iron Will and Toughness are good choices.

**Alternatives:**
- **Toughness** — +1 HP/level (retroactive). Delays Disabled threshold and helps survive Massive Damage. Good choice if Battered is not in play.
- **Hack (Combat)** — Prereqs: Str 13, Power Attack [yes]. Free extra melee attack at full BAB when a foe is KO'd, once per round. Strong synergy with Flurry (more attacks = more KOs). Thematic for a dockside brawler fighting crowds. Can be tested via Martial Flexibility before committing.
- **Improved Overpowering Maneuvers** (if not taken at level 2 or 5 bonus feat)

---

# LEVEL 8 — BRAWLER 7

## Class Features
- **Maneuver Training 2** (flurry attack penalty reduced to -2, up from -1)
- ~~Bonus Feat~~ — **Brawler 7 does NOT grant a bonus feat.** Next bonus combat feat is at Brawler 8 (character level 9).

---

# LEVEL 9 — BRAWLER 8

## Class Features
- **Bonus Combat Feat**
- **Brawler's Flurry upgrade** (now functions as Improved Two-Weapon Fighting — second iterative at -5)

## Feat (Level 9 — regular)
**Recommended: Weapon Focus (Unarmed Strike)**
- Prereqs: proficiency, BAB +1 [yes]. +1 to hit with unarmed strikes.

OR: situational control feat

## Feat (Bonus Combat Feat)
**Recommended: Defensive Mobility** (if not already taken) — Prereqs: Dex 13, Defensive Combat Training [yes]

OR: remaining control feat from the priority list.

---

# LEVEL 10 — BRAWLER 9

## Class Features
- **AC Bonus +2** (dodge bonus; campaign 1.5× multiplier applies if chosen)
- **Brawler's Strike (cold iron & silver)** — unarmed strikes bypass these DR types
- **Knockout 2/day** (up from 1/day)

---

# BEYOND LEVEL 10 — ADDITIONAL BARBARIAN LEVELS

After Brawler 8's Flurry upgrade at level 9, further Barbarian levels are the most impactful dips available. Brawler 9-10 features (AC +2, Knockout 2/day, free-action Martial Flexibility) are solid but can be deferred in favor of Barbarian progression.

## Barbarian 2 (Character Level 10 or 11)
- **Uncanny Dodge** — never flat-footed, retains DEX to AC vs invisible attackers
- **Rage Power** — first selection (see recommendations below)
- If taken at level 10: delays Brawler 9 (AC +2, Knockout 2/day). If taken at level 11: delays Brawler 10 (free-action Martial Flexibility)

## Barbarian 3 (Character Level 11 or 12)
- **Trap Sense +1** — negligible; this is a filler level on the way to Barbarian 4

## Barbarian 4 (Character Level 12 or 13)
- **Rage Power** — second selection
- **DR 1/—** (campaign revision) — stacks with Studded Leather's DR 1/— for **DR 2/—** against all physical damage

## Recommended Rage Powers

**Priority 1 — Strength Surge:** Add Barbarian level to one CMB check per rage. For a grapple specialist, this guarantees one key grab per fight. Combined with the campaign's Choke (CON damage) and Hold (near-shutdown) grapple options, one guaranteed lock-down per rage is a major asset. Prerequisites: none.

**Priority 2 — Knockback:** Every Power Attack hit triggers a free bull rush attempt. Flurry makes 2-3 attacks per round, each with Power Attack active — that's 2-3 push attempts per round. On a dockside map, pushing enemies off piers, into water, or into environmental hazards is both mechanically strong and thematically perfect. Prerequisites: Power Attack [yes], BAB +6 [yes].

**Alternative — Reckless Abandon:** Trade up to BAB in AC for equal attack bonus each round. At Barbarian 2 (BAB ~+9), that's up to -9 AC / +9 to hit. Useful against high-AC enemies the character is unlikely to dodge anyway. Toggle per round based on situation.

**Alternative — Renewed Vigor:** Heal 1d8 + CON HP as a standard action once per rage. Emergency sustain for a frontline fighter who runs out of external healing.

---

# FEAT PRIORITY SUMMARY

## Core Feats (regular levels: 1, 3, 5, 7, 9)
| Level | Feat | Notes |
|-------|------|-------|
| 1 | Power Attack | Str 13, BAB +1 |
| 3 | Defensive Combat Training | No prereqs; +2 DCM, prereq for Defensive Mobility |
| 5 | Catch Off-Guard | BAB +4 (campaign req) |
| 7 | Iron Will | +2 Will saves; offsets Battered penalty |
| 9 | Weapon Focus (Unarmed) | BAB +1, proficiency |

## Bonus Combat Feats (Brawler levels: 2, 5, 8 = character levels 2, 5, 9)
| Char Lvl | Brawler Lvl | Recommended |
|----------|-------------|-------------|
| 2 | Brawler 2 | Improved Grapple |
| 5 | Brawler 5 | Improved Overpowering Maneuvers |
| 9 | Brawler 8 | Defensive Mobility |

## Control Package (campaign custom feats)
- **Improved Expertise Maneuvers** — Int 13 (Brawler's Cunning), Combat Expertise
- **Improved Overpowering Maneuvers** — Str 13, Power Attack, BAB +1
- **Defensive Mobility** — Dex 13, Defensive Combat Training

## Defense Package
- **Iron Will** — +2 Will saves
- **Toughness** (alternative to Iron Will) — +1 HP/level

## Damage Package
- **Power Attack** — Str 13, BAB +1
- **Hack** — Str 13, Power Attack. Extra attack on KO. Alternative to Iron Will at level 7.
- **Weapon Focus (Unarmed Strike)** — BAB +1

## Skip / Not Recommended

- **Combat Reflexes** — With the campaign version using lower of Dex/Int, a Lizardfolk with -2 Int needs 2+ ASIs in INT just to get 1 extra AoO. The base 1 AoO/round is sufficient. Only consider this if starting with INT 14+ (before racial) and willing to invest further.

---

# PREREQUISITE CHECKLIST

| Feat | Prereqs | Key Stat | Viable for Lizardfolk? |
|------|---------|----------|------------------------|
| Power Attack | Str 13, BAB +1 | Str 13+ | Easy (+2 Str) |
| Improved Grapple | Dex 13, IUS | Dex 13+ | Need 13 Dex |
| Imp. Overpowering Maneuvers | Str 13, Power Attack, BAB +1 | Str 13+ | Easy |
| Imp. Expertise Maneuvers | Int 13, Combat Expertise | Int 13 | Brawler's Cunning satisfies Int 13 |
| Defensive Combat Training | None | — | Always |
| Catch Off-Guard | BAB +4 | — | Not before level 5 |
| Hack | Str 13, Power Attack | Str 13+ | Easy |
| Iron Will | None | — | Always |
| Defensive Mobility | Dex 13, Def. Combat Training | Dex 13+ | Need 13 Dex |
| Weapon Focus | Proficiency, BAB +1 | — | Always |
| Toughness | None | — | Always (alternative to Iron Will) |

---

# COMBAT MODES

## Flurry Mode
- Multi-attack pressure (TWF chain via class)
- Default combat style from level 2+
- Upgrades to Improved TWF at Brawler 8 (character level 9)

## Slam Mode
- Single high-damage strike (1d8 + 1.5× STR, scales to 1d10 at Brawler 6)
- Full-round action until Brawler 7, then standard action
- Used vs armored or high-HP targets

## Control Mode
- Grapple / trip / maneuver focus
- Use Martial Flexibility to flex into situational maneuver feats
- Used to deny enemy actions

## Rage Mode
- Barbarian burst: +4 Str/Con, +2 Will, -2 AC
- 4 + Con modifier rounds/day (6 at level 1 with CON 14, scaling with CON increases)
- Used in boss fights or emergencies
- With Strength Surge (Barb 2): one guaranteed grapple per rage — use when a critical target must be locked down
- With Knockback (Barb 2): every Power Attack hit pushes — reposition enemies into hazards or off ledges at will

---

# KEY CAMPAIGN RULES TO REMEMBER

- **Melee attack = BAB + STR + DEX + size** (campaign adds DEX — core is STR only). Dex is twice as valuable.
- **OCM/DCM system** replaces standard CMB/CMD. OCM = BAB+STR+size, DCM = BAB+DEX+size, CMC = 10+BAB+STR+DEX. See `rules_summary.md` for details.
- **Flanked:** Only flanked when threatened by more opponents than attacks from BAB (2+ needed at BAB +1-5, **3+ needed at BAB +6+**). Huge defensive buff.
- **Ability score increases** at levels 4, 6, 8, 10 (every 2 levels, not every 4). Faster stat growth.
- **Armour grants DR** — Studded Leather recommended: +3 AC, DR 1/—, Max DEX +5, works with Brawler AC Bonus.
- **Battered:** < 50% HP = -2 all saves. **Note: not enforced in previous campaign.** If omitted again, Iron Will is still good (poor base Will + Inferior Vision), but Toughness is equally viable at level 7.
- **Combat Reflexes skipped** — needs higher of Dex AND Int. Lizardfolk -2 Int makes it too expensive. Base 1 AoO/round is enough.
- **Catch Off-Guard** requires BAB +4 (not level 1 viable).
- **Improved Grapple** adds Dex 13 prerequisite.
- **Brawler's Slam** is full-round until Brawler 7, then standard action.
- **Martial Flexibility** upgrades at Brawler 6 (swift action for 2 feats).
- Lizardfolk **lacks claws** in Legendspring campaign version (core Lizardfolk has them).
- Lizardfolk has **Inferior Vision** (-4 saves vs illusions, -2 to illusion DCs) and **Low-light Vision** (see twice as far in dim light).
